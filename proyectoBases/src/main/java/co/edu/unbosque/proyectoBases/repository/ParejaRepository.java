...
public interface ParejaRepository extends JpaRepository<Pareja, Integer> {
	...
	@Query(value = "SELECT COALESCE(SUM(cupo_asignado), 0) FROM PAREJA WHERE id_cliente = ?1 AND id_pareja <> ?2", nativeQuery = true)
	double obtenerCupoAsignadoTotalExcluyendo(int idCliente, int idParejaExcluir);
